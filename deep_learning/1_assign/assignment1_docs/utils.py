...
def load_mnist_trainval():
    ...
    # find the index cutoff
    index = int(round(len(data) * 0.8, 0))
    # apply to each array
...
def generate_batched_data(data, label, batch_size=32, shuffle=False, seed=None):
    ...
    # apply to each - VERY important to confirm that each batch is a numpy array
...
def plot_curves(train_loss_history, train_acc_history, valid_loss_history, valid_acc_history):
    ...
    plt.plot(valid_loss_history, "r-", label="val")
    ...
    plt.plot(valid_acc_history, "r-", label="val")
    plt.legend(loc="upper right")
    plt.show()
