...
class Image_Mosaic(object):
    ...
    def image_warp_inv(self, im_src, im_dst, H):
        '''
        Input -
        :param im_src: Image to warp (destination image being stitched)
        :param im_dst: Canvas image (source image, defines output size)
        :param H: 3x3 homography matrix mapping im_src coords -> im_dst coords
        Output -
        :return: im_src warped onto a canvas the same size as im_dst, as uint8
        '''
        h_out, w_out = im_dst.shape[:2]

        # Inverse warp: for each output pixel, find where it maps back to in im_src.
        # H maps im_src -> im_dst, so H_inv maps im_dst -> im_src.
        # For each (x, y) in output, compute H_inv @ [x, y, 1] to get source coords.
        H_inv = np.linalg.inv(H)

        # Build pixel coordinate grids over the output canvas
        ys, xs = np.meshgrid(np.arange(h_out), np.arange(w_out), indexing='ij')
        ones = np.ones((h_out, w_out))
        coords = np.stack([xs, ys, ones], axis=0).reshape(3, -1)  # 3 x (h*w)

        # Map output coords back to im_src space
        src_coords = H_inv @ coords
        src_coords /= src_coords[2, :]  # normalize homogeneous
        map_x = src_coords[0, :].reshape(h_out, w_out).astype(np.float32)
        map_y = src_coords[1, :].reshape(h_out, w_out).astype(np.float32)

        # Sample im_src at those coordinates
        im_src_u8 = im_src.astype(np.uint8)
        if len(im_src.shape) == 3:
            warped = np.zeros((h_out, w_out, im_src.shape[2]), dtype=np.uint8)
            for c in range(im_src.shape[2]):
                dst_ch = np.zeros((h_out, w_out), dtype=np.uint8)
                cv2.remap(im_src_u8[:, :, c], map_x, map_y,
                          cv2.INTER_LINEAR, dst=dst_ch,
                          borderMode=cv2.BORDER_CONSTANT, borderValue=0)
                warped[:, :, c] = dst_ch
        else:
            warped = np.zeros((h_out, w_out), dtype=np.uint8)
            cv2.remap(im_src_u8, map_x, map_y,
    ...
    def output_mosaic(self, img_src, img_warped):
        '''
        Input -
        :param img_src: Source/canvas image
        :param img_warped: Warped destination image (output of image_warp_inv)
        Output -
        :return: Mosaic: warped image placed over source, as uint8
        '''
        img_src_u8 = img_src.astype(np.uint8)
        img_warped_u8 = img_warped.astype(np.uint8)

        # Wherever the warped image has non-zero pixels, use it; otherwise use source
        if len(img_warped_u8.shape) == 3:
            mask = np.any(img_warped_u8 != 0, axis=2)  # (h, w) bool
            mask3 = mask[:, :, np.newaxis]
            mosaic = np.where(mask3, img_warped_u8, img_src_u8)
        else:
            mask = img_warped_u8 != 0
            mosaic = np.where(mask, img_warped_u8, img_src_u8)
