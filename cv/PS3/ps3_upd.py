...
class Image_Mosaic(object):
    ...
    def image_warp_inv(self, im_src, im_dst, H):
        '''
        Input -
        :param im_src: Image 1 (image to warp/sample from)
        :param im_dst: Image 2 (defines output canvas size)
        :param H: numpy ndarray - 3x3 homography matrix (maps im_src coords -> im_dst coords)
        Output -
        :return: Inverse Warped Resulting Image (im_src warped onto im_dst frame)
        '''
        # Output canvas size matches im_dst
        hgtout, wgtout = im_dst.shape[:2]

        # Inverse warp: for each pixel in output, find where it came from in im_src
        # H maps im_src -> im_dst, so H_inv maps im_dst -> im_src
        hinvhomo = np.linalg.inv(H)

        # Build coordinate grids over the output (im_dst) frame
        yyycoords, xxxcoords = np.meshgrid(np.arange(hgtout), np.arange(wgtout), indexing='ij')
        stackcoords = np.stack([xxxcoords.ravel(), yyycoords.ravel(), np.ones(hgtout * wgtout)], axis=0)

        # Apply inverse homography to get source coordinates in im_src
        srccoords = hinvhomo @ stackcoords
        # Normalize homogeneous coords
        srccoords = srccoords / srccoords[2, :]
        srcxxx = srccoords[0, :].reshape(hgtout, wgtout).astype(np.float32)
        srcyyy = srccoords[1, :].reshape(hgtout, wgtout).astype(np.float32)

        # Sample from im_src using the computed source coords
        # Use dst keyword arg as required (see README FAQ on cv2.remap)
        im_src_uint8 = im_src.astype(np.uint8)
        if len(im_src.shape) == 3:
            warpedimg = np.zeros((hgtout, wgtout, im_src.shape[2]), dtype=np.uint8)
            for ccc in range(im_src.shape[2]):
                dst_ch = np.zeros((hgtout, wgtout), dtype=np.uint8)
                cv2.remap(
                    im_src_uint8[:, :, ccc],
                    srcxxx,
                    srcyyy,
            ...
            warpedimg = np.zeros((hgtout, wgtout), dtype=np.uint8)
            cv2.remap(
                im_src_uint8,
                srcxxx,
                srcyyy,
