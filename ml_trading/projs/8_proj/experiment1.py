...
def generate_plot(stock, manual, benchmark, strategy):
    """
    ...
    ax.plot((manual), color='red')
    ax.plot((benchmark), color='purple')
    ax.plot((strategy))
    ax.grid()
    ax.set_title(f'{stock} Strategy Comparison')
    ax.set_xlabel('Date')
    ax.set_ylabel('Normalized Portfolio Value')
    ax.legend(['Manual Strategy', 'Benchmark', 'Strategy Learner'])
    plt.savefig(f'{stock}_strategy_comparison.png')
    plt.close(fig)  
