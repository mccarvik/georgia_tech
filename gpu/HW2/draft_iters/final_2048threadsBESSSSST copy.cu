...
void bitonic_global(DTYPE *__restrict__ arr, int j, int k, int n) {
    ...
    const unsigned int stride = total * 8;  // Increased from 4 to 8

    for (unsigned int base = blockIdx.x * blockDim.x + threadIdx.x;
         base < (unsigned)n;
         base += stride) {

        #pragma unroll 8  // Increased from 4 to 8
        for (int m = 0; m < 8; ++m) {
...
__global__ __launch_bounds__(2048, 1)  // 2048 threads per block
void bitonic_shared(DTYPE *__restrict__ arr, int k, int j_start, int n) {
    extern __shared__ DTYPE s[];
    constexpr int LOCAL = 16384;
    constexpr int THREADS = 2048;  // Double the threads
    constexpr int ELEM_PER_T = 8;   // Half the elements per thread
...
void bitonic_sort() {
    ...
    const int THREADS = 2048;  // 2048 threads per block
