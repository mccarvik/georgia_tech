All code is located at this location on my github, the repo is public:
https://github.com/mccarvik/georgia_tech/tree/master/machine_learning/1_assign

As a backup, I am including a zip of this repo in this box location:
https://gatech.box.com/s/07q895l2mibz0meulrbrhosz5ysmnn52

I am included a zip of the code as well called "1_assign" in case there are any issues accessing the github

The data may not be committed to save space on the remote location. Those files can be found here on kaggle:
MNIST: https://www.kaggle.com/code/datasniffer/raw-pixel-image-data-mnist/input
GTZAN: https://www.kaggle.com/datasets/andradaolteanu/gtzan-dataset-music-genre-classification/data

All code is in jupyter notebook form and uses popular public libraries. 
Each cell has a boolean at the top that is mostly set to false to skip cells I have already used on reruns. Those will have to be set to true to rerun the code. 
There is a jupyter notebook for both the MNIST and the GTZAN and then a third notebook for the neural network tuning called colab_nn.ipynb 
that I used to tune the nn hyperparameters on google colab to utilize their GPUs. 


The libraries needed were very popular libraries. I am listing them here, though there may be some subdependent libraries necessary as well.
I dont see any issues arising from those though:
numpy
pandas
scipy
sklearn
keras
tensorflow
matplotlib
seaborn
xgboost
scikeras


There are a handful of less popular libraries in the imports that are not necessary for the code to run. I have left them in for the sake of completeness.
They were used for preliminary data exploration. Here are those:
librosa
IPython
python_speech_features
tempfile

Please let me know if you have any questions or trouble running.


Finally I am including a pip freeze for completeness as well:
absl-py==1.4.0
accelerate==0.24.1
aiobotocore==2.5.4
aiofiles==23.2.1
aiohttp==3.8.4
aioitertools==0.11.0
aiosignal==1.3.1
ale-py==0.8.1
altair==5.2.0
annotated-types==0.5.0
ansicon==1.89.0
anthropic==0.3.6
anyio==3.7.1
apache-beam==2.50.0
app==0.0.1
appdirs==1.4.4
archspec @ file:///croot/archspec_1697725767277/work
argcomplete==1.10.3
argon2-cffi==21.3.0
argon2-cffi-bindings==21.2.0
arrow==1.2.3
arxiv==2.1.0
asgiref==3.7.2
asn1crypto==1.5.1
astor==0.8.1
asttokens==2.4.1
astunparse==1.6.3
async-lru==2.0.4
async-timeout==4.0.3
atari-py==0.2.9
attr==0.3.2
attrs==23.1.0
audioread==3.0.1
autokeras==1.0.20
AutoROM==0.4.2
AutoROM.accept-rom-license==0.6.1
Babel==2.12.1
backcall==0.2.0
backoff==2.2.1
bcrypt==4.1.2
beautifulsoup4==4.12.2
bert-extractive-summarizer==0.7.0
bitsandbytes==0.41.3.post2
bleach==6.0.0
blessed==1.20.0
blinker==1.7.0
blis==0.7.10
boltons==21.0.0
boto3==1.28.20
botocore==1.31.85
bracex==2.4
breadability==0.1.20
Brotli @ file:///C:/ci_311/brotli-split_1676435766766/work
bs4==0.0.1
cachetools==5.3.1
camel-converter==3.1.1
catalogue==2.0.9
certifi==2023.7.22
cffi==1.15.1
channels==4.0.0
chardet==3.0.4
charset-normalizer==3.2.0
chroma-hnswlib==0.7.3
chromadb==0.4.21
click==8.1.7
click-option-group==0.5.6
cloudpickle==2.2.1
colorama @ file:///C:/ci_311/colorama_1676422310965/work
coloredlogs==15.0.1
comm==0.2.1
compressed-rtf==1.0.6
conda @ file:///C:/b/abs_f5n1kle6av/croot/conda_1699303158682/work
conda-content-trust @ file:///C:/b/abs_e3bcpyv7sw/croot/conda-content-trust_1693490654398/work
conda-libmamba-solver @ file:///C:/b/abs_16rwdkt1_p/croot/conda-libmamba-solver_1698961834174/work/src
conda-package-handling @ file:///C:/b/abs_b9wp3lr1gn/croot/conda-package-handling_1691008700066/work
conda_package_streaming @ file:///C:/b/abs_6c28n38aaj/croot/conda-package-streaming_1690988019210/work
confection==0.1.1
contourpy==1.1.0
crcmod==1.7
crypto==1.4.1
cryptography==41.0.3
cssutils==2.9.0
curl-cffi==0.5.10
cycler==0.11.0
cymem==2.0.7
Cython==3.0.0
dataclasses==0.6
dataclasses-json==0.5.14
dataframe-image==0.2.3
datasets==2.14.5
dateparser==1.1.8
debugpy==1.8.0
decorator==5.1.1
defusedxml==0.7.1
demoji==1.1.0
Deprecated==1.2.14
dill==0.3.1.1
distro==1.9.0
Django==5.0
dm-tree==0.1.8
dnspython==2.4.2
docarray==0.32.1
docker-pycreds==0.4.0
docopt==0.6.2
docx2txt==0.8
duckdb==0.8.1
duckduckgo_search==4.1.1
EasyProcess==1.1
ebcdic==1.1.1
elastic-transport==8.4.0
elasticsearch==8.9.0
emoji==1.2.0
et-xmlfile==1.1.0
executing==2.0.1
extract-msg==0.28.7
face==22.0.0
faiss-cpu==1.7.4
Faker==19.13.0
Farama-Notifications==0.0.4
fastai==2.7.12
fastai2==0.0.30
fastapi==0.103.2
fastavro==1.8.3
fastcore==1.5.29
fastdownload==0.0.7
fasteners==0.19
fastjsonschema==2.18.0
fastprogress==1.0.3
feedparser==6.0.10
filelock==3.12.2
flatbuffers==23.5.26
fonttools==4.42.0
fqdn==1.5.1
frozenlist==1.4.0
fsspec==2023.9.2
gast==0.4.0
gcsfs==2023.9.2
gensim==4.3.1
git-python==1.0.3
gitdb==4.0.11
gitignore_parser==0.1.9
GitPython==3.1.40
glom==22.1.0
google-api-core==2.12.0
google-auth==2.22.0
google-auth-oauthlib==1.0.0
google-cloud-aiplatform==1.38.1
google-cloud-bigquery==3.14.1
google-cloud-core==2.3.3
google-cloud-resource-manager==1.11.0
google-cloud-storage==2.11.0
google-crc32c==1.5.0
google-pasta==0.2.0
google-resumable-media==2.6.0
google-search-results==2.4.2
googleapis-common-protos==1.60.0
googledrivedownloader==0.4
gpt4all==2.0.2
graphviz==0.20.1
greenlet==2.0.2
grpc-google-iam-v1==0.13.0
grpcio==1.60.0
grpcio-status==1.60.0
grpcio-tools==1.60.0
gym==0.26.2
gym-notices==0.0.8
gymnasium==0.29.1
h11==0.14.0
h2==4.1.0
h5py==3.9.0
hdfs==2.7.2
hnswlib==0.8.0
hpack==4.0.0
html2image==2.0.4.3
httpcore==0.17.3
httplib2==0.22.0
httptools==0.6.1
httpx==0.24.1
httpx-sse==0.3.1
huggingface-hub==0.20.1
humanfriendly==10.0
humanize==4.9.0
hyperframe==6.0.1
idna==3.4
ignite==1.1.0
imageio==2.31.1
imageio-ffmpeg==0.4.9
IMAPClient==2.1.0
importlib-metadata==6.8.0
importlib-resources==6.1.0
iniconfig==2.0.0
inquirer==3.1.4
ipdb==0.13.13
ipykernel==6.29.0
ipython==8.20.0
ipython-genutils==0.2.0
ipywidgets==8.1.0
isodate==0.6.1
isoduration==20.11.0
jedi==0.19.1
jinaai==0.2.9
Jinja2==3.1.2
jinxed==1.2.1
jmespath==1.0.1
joblib==1.2.0
json5==0.9.14
jsonpatch==1.33
jsonpointer==2.4
jsonschema==4.20.0
jsonschema-path==0.3.2
jsonschema-spec==0.2.4
jsonschema-specifications==2023.7.1
jupyter==1.0.0
jupyter-console==6.6.3
jupyter-events==0.7.0
jupyter-lsp==2.2.0
jupyter_client==8.6.0
jupyter_core==5.7.1
jupyter_server==2.7.0
jupyter_server_terminals==0.4.4
jupyterlab==4.0.4
jupyterlab-pygments==0.2.2
jupyterlab-widgets==3.0.8
jupyterlab_server==2.24.0
keras==3.0.4
keras-tuner==1.3.5
kiwisolver==1.4.4
kt-legacy==1.0.5
kubernetes==28.1.0
lanarky==0.8.4
lancedb==0.1.16
langchain==0.0.353
langchain-community==0.0.7
langchain-core==0.1.4
langchain-decorators==0.4.2
langchain-experimental==0.0.23
langchainplus-sdk==0.0.20
langcodes==3.3.0
langsmith==0.0.75
lazy-object-proxy==1.10.0
lazy_loader==0.3
lazypredict==0.2.12
libclang==16.0.6
libcst==1.0.1
libmambapy @ file:///C:/b/abs_efpsdwt4ya/croot/mamba-split_1698782663578/work/libmambapy
librosa==0.10.1
lightgbm==4.0.0
lightning-utilities==0.9.0
lime==0.2.0.1
litellm==0.1.824
llvmlite==0.40.1
loguru==0.7.2
lxml==4.9.3
Markdown==3.4.4
markdown-it-py==3.0.0
MarkupSafe==2.1.3
marshmallow==3.20.1
matplotlib==3.7.2
matplotlib-inline==0.1.6
mdurl==0.1.2
meilisearch==0.21.0
menuinst @ file:///C:/ci_311/menuinst_1678730372782/work
metagpt==0.5.2
mistune==3.0.1
mlxtend==0.23.0
mmh3==4.0.1
monotonic==1.6
more-itertools==10.1.0
moviepy==1.0.3
mpmath==1.3.0
msgpack==1.0.7
multidict==6.0.4
multiprocess==0.70.15
murmurhash==1.0.9
mwparserfromhell==0.6.5
mypy-extensions==1.0.0
Naked==0.1.32
namex==0.0.7
nbclient==0.8.0
nbconvert==7.7.3
nbformat==5.9.2
nest-asyncio==1.6.0
netaddr==0.9.0
networkx==3.1
nltk==3.8.1
notebook==7.0.2
notebook_shim==0.2.3
num2words==0.5.12
numba==0.57.1
numexpr==2.8.6
numpy==1.24.3
oauthlib==3.2.2
objsize==0.6.1
olefile==0.46
onnxruntime==1.16.3
open-interpreter==0.1.7
openai==0.28.1
openapi-core==0.18.2
openapi-schema-pydantic==1.2.4
openapi-schema-validator==0.6.2
openapi-spec-validator==0.7.1
opencv-python==4.8.1.78
openpyxl==3.1.2
opentelemetry-api==1.22.0
opentelemetry-exporter-otlp-proto-common==1.22.0
opentelemetry-exporter-otlp-proto-grpc==1.22.0
opentelemetry-instrumentation==0.43b0
opentelemetry-instrumentation-asgi==0.43b0
opentelemetry-instrumentation-fastapi==0.43b0
opentelemetry-proto==1.22.0
opentelemetry-sdk==1.22.0
opentelemetry-semantic-conventions==0.43b0
opentelemetry-util-http==0.43b0
openvpn-api==0.3.0
openvpn-status==0.2.2
opt-einsum==3.3.0
orjson==3.9.7
oscrypto==1.3.0
overrides==7.4.0
packaging @ file:///C:/b/abs_28t5mcoltc/croot/packaging_1693575224052/work
pandas==2.2.0
pandasai==1.5.12
pandocfilters==1.5.0
parse==1.20.0
parso==0.8.3
pathable==0.4.3
pathy==0.10.2
patsy==0.5.3
pdfminer.six==20191110
peewee==3.17.0
peft==0.7.1
pickleshare==0.7.5
Pillow==10.1.0
pinecone-client==2.2.4
pinecone-datasets==0.6.1
pip-autoremove==0.10.0
platformdirs==4.1.0
pluggy @ file:///C:/ci_311/pluggy_1676422178143/work
pooch==1.8.0
portalocker==2.8.2
posthog==3.1.0
prance==23.6.21.0
preshed==3.0.8
proglog==0.1.10
prometheus-client==0.17.1
prompt-toolkit==3.0.43
promptwatch==0.4.3
proto-plus==1.22.3
protobuf==4.23.4
psutil==5.9.8
psycopg2==2.9.7
ptan==0.4
pulsar-client==3.3.0
pure-eval==0.2.2
py==1.11.0
pyarrow==11.0.0
pyasn1==0.5.0
pyasn1-modules==0.3.0
pybind11==2.11.1
pybullet==3.2.5
pycorenlp==0.3.0
pycosat @ file:///C:/b/abs_31zywn1be3/croot/pycosat_1696537126223/work
pycountry==22.3.5
pycparser==2.21
pycryptodome==3.18.0
pycryptodomex==3.18.0
pydantic==1.10.8
pydantic_core==2.4.0
pydeck==0.8.1b0
pydot==1.4.2
pydotplus==2.0.2
pygame==2.5.2
Pygments==2.17.2
PyJWT==2.8.0
pylance==0.5.10
pymongo==4.5.0
PyMuPDF==1.23.8
PyMuPDFb==1.23.7
pyOpenSSL==23.2.0
pyparsing==3.0.9
pypdf==3.17.4
PyPika==0.48.9
pypiwin32==223
PyPrind==2.11.3
pyreadline3==3.4.1
PySocks @ file:///C:/ci_311/pysocks_1676425991111/work
pytest==7.2.2
pytest-mock==3.11.1
python-crfsuite==0.9.9
python-dateutil==2.8.2
python-docx==0.8.11
python-dotenv==1.0.0
python-editor==1.0.4
python-json-logger==2.0.7
python-pptx==0.6.21
python-speech-features==0.6
pytorch-ignite==0.4.12
pytorch-lightning==2.1.0
pytorch-nlp==0.5.0
pytorch-pretrained-bert==0.6.2
pytorch-transformers==1.2.0
pytz==2023.3
PyVirtualDisplay==0.2.5
PyWavelets==1.4.1
pywin32==306
pywinpty==2.0.11
PyYAML==6.0.1
pyzmq==25.1.2
qdrant-client==1.4.0
qtconsole==5.4.3
QtPy==2.3.1
ratelimiter==1.2.0.post0
rdflib==7.0.0
readchar==4.0.5
referencing==0.30.2
regex==2023.6.3
replicate==0.22.0
requests==2.31.0
requests-oauthlib==1.3.1
retry==0.9.2
rfc3339-validator==0.1.4
rfc3986-validator==0.1.1
rich==13.7.0
rpds-py==0.9.2
rsa==4.9
ruamel.yaml @ file:///C:/ci_311/ruamel.yaml_1676439214109/work
s3fs==2023.9.2
s3transfer==0.6.1
sacremoses==0.0.53
safetensors==0.3.2
scikeras==0.12.0
scikit-image==0.21.0
scikit-learn==1.2.0
scipy==1.11.1
seaborn==0.12.2
semantic-kernel==0.3.13.dev0
semgrep==1.52.0
semver==3.0.2
Send2Trash==1.8.2
sentencepiece==0.1.99
sentry-sdk==1.39.1
seqeval==1.2.2
setproctitle==1.3.3
sgmllib3k==1.0.0
shap==0.42.1
shapely==2.0.2
shellescape==3.8.1
six==1.16.0
sklearn==0.0.post7
sklearn-crfsuite==0.3.6
slicer==0.0.7
smart-open==6.3.0
smmap==5.0.1
sniffio==1.3.0
snowflake-connector-python==3.1.0
sortedcontainers==2.4.0
soundfile==0.12.1
soupsieve==2.4.1
soxr==0.3.7
spacy==3.6.1
spacy-legacy==3.0.12
spacy-loggers==1.0.4
SpeechRecognition==3.8.1
SQLAlchemy==1.4.50
sqlparse==0.4.4
srsly==2.4.7
sse-starlette==1.8.2
stack-data==0.6.3
starlette==0.27.0
statsmodels==0.14.0
streamlit==1.29.0
summa==1.2.0
sumy==0.11.0
sympy==1.12
ta==0.10.2
tabulate==0.9.0
tenacity==8.2.2
tensorboard==2.14.0
tensorboard-data-server==0.7.1
tensorboardX==2.6.2.2
tensorflow==2.13.0
tensorflow-estimator==2.13.0
tensorflow-intel==2.13.0
tensorflow-io-gcs-filesystem==0.31.0
termcolor==2.3.0
terminado==0.17.1
textblob==0.17.1
textract==1.6.5
thinc==8.1.12
threadpoolctl==3.2.0
tifffile==2023.8.12
tiktoken==0.4.0
tinycss2==1.2.1
tokenizers==0.13.3
tokentrim==0.1.13
toml==0.10.2
tomli==2.0.1
tomlkit==0.12.1
toolz==0.12.0
torch==2.1.2
torch-geometric==2.0.2
torch-scatter==2.1.2
torch-sparse==0.6.18
torchdata==0.7.0
torchmetrics==1.2.0
torchtext==0.16.0
torchvision==0.15.2
tornado==6.4
tqdm==4.64.0
traitlets==5.14.1
transformers==4.31.0
truststore @ file:///C:/b/abs_55z7b3r045/croot/truststore_1695245455435/work
tweepy==4.14.0
twikenizer==1.0
typer==0.9.0
types-requests==2.31.0.10
typing-inspect==0.8.0
typing_extensions==4.5.0
tzdata==2023.3
tzlocal==5.0.1
Unidecode==1.3.6
unzip==1.0.0
uri-template==1.3.0
urllib3==1.26.16
uvicorn==0.23.2
vaderSentiment==3.3.2
validators==0.22.0
vertexai==0.0.1
wandb==0.16.1
wasabi==1.1.2
watchdog==3.0.0
watchfiles==0.21.0
watermark==2.4.3
wcmatch==8.5
wcwidth==0.2.13
webcolors==1.13
webencodings==0.5.1
websocket-client==0.58.0
websockets==11.0.3
Werkzeug==2.3.6
wget==3.2
widgetsnbextension==4.0.8
wikipedia==1.4.0
win-inet-pton @ file:///C:/ci_311/win_inet_pton_1676425458225/work
win32-setctime==1.1.0
wordcloud==1.9.2
wrapt==1.15.0
xformers==0.0.23.post1
xgboost==1.7.6
xlrd==2.0.1
XlsxWriter==3.1.2
xmltodict==0.13.0
xxhash==3.3.0
yacs==0.1.8
yahoofinancials==1.16
yarl==1.9.2
yaspin==3.0.1
zep-python==1.5.0
zhipuai==1.0.7
zipp==3.17.0
zstandard==0.21.0